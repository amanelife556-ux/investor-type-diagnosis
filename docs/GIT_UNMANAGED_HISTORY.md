...
## 2026-05-06 - スマホタイトル画像の表示崩れと初回表示速度を改善
...
変更内容:
- タイトル専用画像を `1536 x 1024` PNGから、表示用の `1200 x 800` WebPへ軽量化。
- `assets/title-hero.png` は元画像として残し、アプリでは `assets/title-hero.webp` を直接読み込むように変更。
- タイトル画像を比率固定コンテナに入れ、スマホで上部だけ表示されて下に空白が出る問題を修正。
- スマホではタイトル画像の枠を本文幅より広げ、画面幅に近い大きさで表示するよう調整。

理由:
- iPhoneや初回ロード時に重いPNGの描画が追いつかず、タイトル画像が途中表示に見える可能性が高かったため。
- タイトル画面の第一印象を強めるため。

確認:
- `assets/title-hero.webp`: `1200 x 800`, 約423KB
- スマホ幅 `390 x 844` のスクリーンショットで、画像全体が表示されることを確認。
- `npm run publish:check`: 成功
  - `npm run build`: 成功
  - `npm test`: 48 tests passed
  - `npm run test:logic`: smoke test passed
...
## 2026-05-06 - Xカード画像キャッシュ対策でOGP画像をv2化
...
変更内容:
- Xで古いOGP画像が表示される問題に対応するため、OGP画像を `og-image-v2.png` として新規作成。
- 新しいOGP画像は、タイトル専用ビジュアルを背景にした構成へ変更。
- `og:image` と `twitter:image` を `https://investor-type-diagnosis.vercel.app/og-image-v2.png` に更新。

理由:
- Xは一度取得したOGP画像を強くキャッシュするため、同じ `og-image.png` を更新しても古い画像が表示されやすい。
- ファイル名を変えることで、Xに新しい画像として取得させやすくするため。

確認:
- `public/og-image-v2.png`: `1200 x 630` PNG

未確認:
- Vercel再デプロイ後のX上のカード再取得。

## 2026-05-06 - 明るさ調整済みの結果カードをactiveへ反映
...
変更内容:
- `assets/share-cards-tone-working/` にあった明るさ調整済み16枚を、アプリ参照先の `assets/share-cards/` に反映。

理由:
- 結果画面、保存画像、X投稿添付用カードに、読みやすく明るくした最新版を使うため。

確認:
- 16枚すべて `1200 x 675` PNGとして存在。
- `assets/share-cards-tone-working/_contact_sheet.png` で一覧確認。
- `npm run publish:check`: 成功
  - `npm run build`: 成功
  - `npm test`: 48 tests passed
  - `npm run test:logic`: smoke test passed
